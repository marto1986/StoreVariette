...
class Souvenirs extends CI_Controller {
	...
	function index(){
		...
		$this->load->view('admin/souvenirs/souvenirs', $data);
	}
	...
	public function nuevo(){
		...
		$this->load->view('admin/header');
		$this->load->view('admin/nav');
		$this->load->view('admin/souvenirs/nuevo');
	}
	...
	function insert(){
		...
		$this->load->library('upload');
	        ...
	        if(!empty($_FILES['foto']['name'])){
		        $config['upload_path'] = './img/souvenirs/';
	    ...
	    $this->souvenirsModel->insert($nombre, $titulo, $subTitulo, $opcional, $descripcion, $condicion, $colores, $pagos, $envios, $consultas, $email, $whatsapp, $tabla, $collage, $precio, $foto, $foto2, $foto3, $foto4, $foto5);
	        
	    header("location:" . base_url() . "souvenirs");
	}
	...
	function editar(){
		...
		if($datos != false){
			...
			$this->load->view('admin/header');
			$this->load->view('admin/nav');
			$this->load->view('admin/souvenirs/editar', $data);
	}
	...
	function actualizar(){
	    ...
	    $this->souvenirsModel->actualizar($id, $nombre, $titulo, $subTitulo, $opcional, $descripcion, $condicion, $colores, $pagos, $envios, $consultas, $email, $whatsapp, $tabla, $collage, $precio, $foto, $foto2, $foto3, $foto4, $foto5);
	...
	function eliminar(){
		$id = $this->uri->segment(3);
		$this->souvenirsModel->eliminar($id);
	...
	function buscar(){
		...
		if($query){
			$result = $this->souvenirsModel->buscar(trim($query));
			$total = $this->souvenirsModel->totalResultados(trim($query));
			if($result != FALSE){
				$data = array(
					'result' => $result,
					'total' => $total
